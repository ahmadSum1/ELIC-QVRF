...
class TestModel(CompressionModel):
    ...
    def forward(self, x, noise=False, stage=3, s=1, sal_map=None):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            if noise:
                y_anchor_quantilized = self.quantizer.quantize(y_anchor * QuantizationRegulator,
                                                               "noise") * ReQuantizationRegulator
                y_anchor_quantilized_for_gs = self.quantizer.quantize(y_anchor * QuantizationRegulator,
                                                                      "ste") * ReQuantizationRegulator
            else:
                y_anchor_quantilized = self.quantizer.quantize((y_anchor - means_anchor) * QuantizationRegulator,
                                                               "ste") * ReQuantizationRegulator + means_anchor
                y_anchor_quantilized_for_gs = self.quantizer.quantize((y_anchor - means_anchor) * QuantizationRegulator,
                                                                      "ste") * ReQuantizationRegulator + means_anchor
            ...
            if noise:
                y_non_anchor_quantilized = self.quantizer.quantize(y_non_anchor * QuantizationRegulator, "noise") * ReQuantizationRegulator
                y_non_anchor_quantilized_for_gs = self.quantizer.quantize(y_non_anchor * QuantizationRegulator, "ste") * ReQuantizationRegulator
            else:
                y_non_anchor_quantilized = self.quantizer.quantize(
                    (y_non_anchor - means_non_anchor) * QuantizationRegulator,
                    "ste") * ReQuantizationRegulator + means_non_anchor
    ...
    def compress(self, x, s, inputscale=0, sal_map=None):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            indexes_anchor = self.gaussian_conditional.build_indexes(scales_anchor_encode*QuantizationRegulator)
            anchor_strings = self.gaussian_conditional.compress(
                (y_anchor_encode - means_anchor_encode) * QuantizationRegulator, indexes_anchor)
            anchor_quantized = self.gaussian_conditional.decompress(anchor_strings,
                                                                    indexes_anchor) * ReQuantizationRegulator + means_anchor_encode
            ...
            indexes_non_anchor = self.gaussian_conditional.build_indexes(scales_non_anchor_encode*QuantizationRegulator)
            non_anchor_strings = self.gaussian_conditional.compress(
                (y_non_anchor_encode - means_non_anchor_encode) * QuantizationRegulator, indexes_non_anchor, )

            non_anchor_quantized = self.gaussian_conditional.decompress(non_anchor_strings,
                                                                        indexes_non_anchor) * ReQuantizationRegulator + means_non_anchor_encode
